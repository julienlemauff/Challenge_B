...
We choose to use Random Forest which is usefull to use it when we have an important number of explanatory variables. It is a machine learning algorithm which is very usefull to spot the links between the dependant vairable and the explanatory ones: Random Forest class them according to their links with the dependant vairable. More technically, it follows the principle of "bagging" which consists to compute the mean of the predictions of several independants models to reduce the prediction error.
...
```{r missing data step 3, echo = TRUE, results='hide'}
train %>% summarise_all(.funs = funs(sum(is.na(.)))) %>% gather(key = "feature", value = "missing.observations") %>% filter(missing.observations > 0)
...
data_test <- read.csv(file = file.choose()) # import the data test.csv
attach(data_test)
...
Which of these two predictions are the best ? We first note that the model "fit" explain 87% of the variance whereas "lm_model_2" has a multiple R-squared of 0,7253. 
...
We can see (in green) that the highflex modeltends to follow all the points (the real observation), so this model gives at the same time the best predictions but also the predictions the more variables. We also know that the estimator's variance is in link the variance of observation, that's why the predictions with the least bias is lowflex.
...
```{r Import the CNIL dataset, echo = FALSE}
data_CNIL <- read_excel("~/rprog1/challenge B/OpenCNIL_Organismes_avec_CIL_VD_20171115.xlsx")
View(data_CNIL)
head(data_CNIL)
attach(data_CNIL)
```
...
```{r Clean the CNIL dataset, echo = TRUE}
remove.na <- data_CNIL %>% summarise_all(.funs = funs(sum(is.na(.)))) %>% gather(key = "feature", value = "missing.observations") %>% filter(missing.observations > 100) %>% select(feature) %>% unlist

data_CNIL <- data_CNIL %>% select(- one_of(remove.na))

data_CNIL %>% summarise_all(.funs = funs(sum(is.na(.)))) %>% gather(key = "feature", value = "missing.observations") %>% filter(missing.observations > 0)

# I eliminate the rows where there are NAs for these three features
data_CNIL <- data_CNIL %>% filter(is.na(Adresse) == FALSE, is.na(`Code Postal`) == FALSE, is.na(Ville) == FALSE)

# And finally I check if it's okay by doing one more time this code : 
data_CNIL %>% summarise_all(.funs = funs(sum(is.na(.)))) %>% gather(key = "feature", value = "missing.observations") %>% filter(missing.observations > 0)

data_CNIL <- data.frame(data_CNIL)
attach(data_CNIL)
```

Then we can show in a nice table the number of organizations that has nominated a CNIL per department.

```{r the nice table, echo = TRUE}
